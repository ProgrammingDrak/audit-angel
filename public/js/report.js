...
function renderSinglePin(pin, idx, invId) {
  ...
  if (pin.images && pin.images.length > 0 && !_imagesHidden) {
    ...
    pin.images.forEach(function(img) {
      ...
      if (img.caption) html += '<div style="font-size:10px;color:#6B7280;margin-top:2px;">' + escHtml(img.caption) + '</div>';
      if (img.link) html += '<div style="font-size:10px;"><a href="' + escHtml(img.link) + '" target="_blank" style="color:#0075EB;text-decoration:none;">' + escHtml(img.link).substring(0, 50) + '...</a></div>';
      html += '</div>';
    });
    html += '</div>';
  }
