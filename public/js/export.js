...
function exportInvHTML() {
  ...
  var html = buildInvestigationReportHTML(inv);
  downloadHTML(getReportFilename(inv, 'html'), html);
}

function buildInvestigationReportHTML(inv) {
...
async function exportInvPDF() {
  if (!_currentReportInv) return;
  if (!window.html2canvas) {
    showToast('PDF export needs html2canvas to finish loading');
    return;
  }
  ...
  try {
    ...
    doc.open();
    doc.write(buildInvestigationReportHTML(inv));
    doc.close();

    await waitForReportFrame(iframe);

    var reportBody = doc.body;
    var canvas = await html2canvas(reportBody, {
      backgroundColor: '#F9FAFB',
      scale: Math.min(2, window.devicePixelRatio || 1.5),
      useCORS: true,
      windowWidth: Math.max(doc.documentElement.scrollWidth, 960),
      windowHeight: Math.max(doc.documentElement.scrollHeight, 1200),
    });
    ...
    pdf.save(getReportFilename(inv, 'pdf'));
    showToast('PDF exported');
  } catch (err) {
    console.error('[export] PDF export failed:', err);
    showToast('PDF export failed: ' + (err && err.message ? err.message : 'unknown error'));
  } finally {
    if (iframe && iframe.parentNode) iframe.parentNode.removeChild(iframe);
  }
}

function getReportFilename(inv, extension) {
  var base = (inv.name || 'investigation')
    .replace(/[^a-zA-Z0-9 _-]/g, '')
    .replace(/\s+/g, '-')
    .toLowerCase();
  return (base || 'investigation') + '-report.' + extension;
}

function ensureJsPdfLoaded() {
  if (window.jspdf && window.jspdf.jsPDF) return Promise.resolve();
  return new Promise(function(resolve, reject) {
    var existing = document.querySelector('script[data-aa-jspdf]');
    if (existing) {
      existing.addEventListener('load', function() { resolve(); }, { once: true });
      existing.addEventListener('error', function() { reject(new Error('Could not load PDF exporter')); }, { once: true });
      return;
    }
    var script = document.createElement('script');
    script.src = 'https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js';
    script.async = true;
    script.setAttribute('data-aa-jspdf', 'true');
    script.onload = function() { resolve(); };
    script.onerror = function() { reject(new Error('Could not load PDF exporter')); };
...
function waitForReportFrame(iframe) {
  return new Promise(function(resolve) {
    ...
    var finish = function() {
      var fontReady = doc.fonts && doc.fonts.ready ? doc.fonts.ready.catch(function(){}) : Promise.resolve();
      fontReady.then(function() {
        return waitForReportImages(doc);
      }).then(function() {
...
function waitForReportImages(doc) {
